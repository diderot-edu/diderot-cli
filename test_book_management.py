...
def test_create_part(api_client, book, pi):
    ...
    url = (MANAGE_BOOK_API + "parts/").format(**route_params)
...
def test_create_chapter(api_client, part, pi):
    ...
    route_params = {"course_id": part.book.course_id, "book_id": part.book_id}
    url = (MANAGE_BOOK_API + "manage-chapters/").format(**route_params)
    data = {"title": "title", "label": "label", "rank": 1, "part": part.id}
    response = api_client.post(url, data)
