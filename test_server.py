    ...
    def do_POST(self):
        ...
        # support book management only on course 0
            ...
            if params['kind'] == 'create chapter':
                ...
                success = all(['part_pk' in params, 'rank' in params,
                               'title' in params, 'label' in params, success])
            elif params['kind'] == 'create part':
                # assert lable, title and rank are present
                success = all(
                    ['rank' in params, 'title' in params, 'label' in params, success])
        ...
        # support homework management only on course 0
            ...
            success = all([success, 'autograde-makefile' in fields,
                           'autograde-tar' in fields, 'writeup' in fields, 'handout' in fields])
            if success:
                self.send_response(200)
            else:
                self.send_response(400)
